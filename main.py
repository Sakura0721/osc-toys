...
async def start_channel_a():
    ...
    await ci.signal(power=int(settings.COYOTE_MAX_POWER_A*settings.MIN_POWER),
...
async def start_channel_b():
    print(ci.patterns[settings.COYOTE_PATTERN])
    await ci.signal(power=int(settings.COYOTE_MAX_POWER_B*settings.MIN_POWER),
                    pattern=ci.patterns[settings.COYOTE_PATTERN],
                    duration=100000000,
                    channel="b")
...
def get_avg(queue : List[float]) -> float:
    ...
    if s <= settings.MIN_LIMIT:
        ...
        s = (s-settings.MIN_LIMIT)/(settings.MAX_LIMIT-settings.MIN_LIMIT)*(1-settings.MIN_POWER)+settings.MIN_POWER
    return s
...
def coyote_handler_a(addr, args, dis):
    ...
    if cur_time_a - last_time_a > settings.WINDOW_SIZE:
        ...
        if s < 0.1:
            ...
            asyncio.ensure_future(ci.set_pwm(
                int(settings.COYOTE_MAX_POWER_A*s), -1), loop=loop)
        param_queue_a = []
    else:
        param_queue_a.append(dis)
...
def coyote_handler_b(addr, args, dis):
    ...
    if cur_time_b - last_time_b > settings.WINDOW_SIZE:
        ...
        if s < 0.1:
            ...
            asyncio.ensure_future(ci.set_pwm(
                -1, int(settings.COYOTE_MAX_POWER_B*s)), loop=loop)
        param_queue_b = []
    else:
        param_queue_b.append(dis)
